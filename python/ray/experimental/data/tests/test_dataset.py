...
from ray.experimental.data.tests.conftest import *  # noqa
...
def test_json_read(ray_start_regular_shared, fs, data_path, endpoint_url):
    ...
    ds = ray.experimental.data.read_json(path1, filesystem=fs)
    ...
    ds = ray.experimental.data.read_json(
        [path1, path2],
        parallelism=2,
        filesystem=fs)
    ...
    df2.to_json(
        path2, orient="records", lines=True, storage_options=storage_options)
    ds = ray.experimental.data.read_json(path, filesystem=fs)
    df = pd.concat([df1, df2])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
    ...
    df3.to_json(
        ...
        storage_options=storage_options)
    ds = ray.experimental.data.read_json([path1, path2], filesystem=fs)
    df = pd.concat([df1, df2, df3])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
    ...
    df2.to_json(
        path2, orient="records", lines=True, storage_options=storage_options)
    ds = ray.experimental.data.read_json([dir_path, path2], filesystem=fs)
    df = pd.concat([df1, df2])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
...
def test_csv_read(ray_start_regular_shared, fs, data_path, endpoint_url):
    ...
    ds = ray.experimental.data.read_csv(path1, filesystem=fs)
    dsdf = ray.get(ds.to_pandas())[0]
    assert df1.equals(dsdf)
    # Test metadata ops.
    assert ds.count() == 3
    ...
    df2.to_csv(path2, index=False, storage_options=storage_options)
    ds = ray.experimental.data.read_csv(path, filesystem=fs)
    df = pd.concat([df1, df2])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
    ...
    ds = ray.experimental.data.read_csv([path1, path2], filesystem=fs)
    df = pd.concat([df1, df2, df3])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
    ...
    df2.to_csv(path2, index=False, storage_options=storage_options)
    ds = ray.experimental.data.read_csv([dir_path, path2], filesystem=fs)
    df = pd.concat([df1, df2])
    dsdf = pd.concat(ray.get(ds.to_pandas()))
    assert df.equals(dsdf)
    if fs is None:
